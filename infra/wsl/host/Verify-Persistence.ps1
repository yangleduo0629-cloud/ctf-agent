...
wsl.exe -d $Distribution -u root -- bash -lc "$root/prepare-restore-fixture.sh '$token'"
...
wsl.exe -d $Distribution -u root -- bash -lc "for i in {1..60}; do $root/health-check.sh --skip-gpu && exit 0; sleep 5; done; exit 1"
if ($LASTEXITCODE -ne 0) { throw 'Services did not recover after WSL restart.' }
wsl.exe -d $Distribution -u root -- bash -lc "$root/verify-fixture.sh '$token'"
